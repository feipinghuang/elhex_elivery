defmodule ElhexDelivery.PostalCode.Navigator do
  ...
  defp do_get_distance(from, to) do
    ...
    case Cache.get_distance(from, to) do
      nil ->
        {lat1, long1} = get_geolacation(from)
        {lat2, long2} = get_geolacation(to)
        distance = caculate_distance({lat1, long1}, {lat2, long2})
  ...
  defp get_geolacation(postal_code) do
    Store.get_geolacation(postal_code)
  end
  ...
  defp caculate_distance({lat1, long1}, {lat2, long2}) do
  ...
  defp degrees_to_radians(degress) do
    degress * (Math.pi / 180)
  end
end
